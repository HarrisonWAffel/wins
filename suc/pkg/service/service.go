...
const (
	AutoStart        StartType = 1
	DelayedAutoStart StartType = 2
)
...
func ConfigureStartType(sType StartType, s *mgr.Service) error {
	cnf, err := s.Config()
	if err != nil {
		return fmt.Errorf("failed to get config for service %s: %v", s.Name, err)
	}
	switch sType {
	case AutoStart:
		cnf.DelayedAutoStart = false
		return s.UpdateConfig(cnf)
	case DelayedAutoStart:
		cnf.DelayedAutoStart = true
		return s.UpdateConfig(cnf)
	default:
		return fmt.Errorf("invalid start type provided")
	}
}

func CreateServiceDependency(parentService string, s *mgr.Service) error {
	cnf, err := s.Config()
	if err != nil {
		return fmt.Errorf("failed to get config for service %s: %v", s.Name, err)
	}
	alreadyAdded := true
	for _, e := range cnf.Dependencies {
		if e == parentService {
			alreadyAdded = false
			break
		}
	...
	if !alreadyAdded {
		cnf.Dependencies = append(cnf.Dependencies, parentService)
		return s.UpdateConfig(cnf)
	}
	return nil
}

func RemoveServiceDependency(parentService string, s *mgr.Service) error {
	cnf, err := s.Config()
	if err != nil {
		return fmt.Errorf("failed to get config for service %s: %v", s.Name, err)
	}
	...
	for _, e := range cnf.Dependencies {
		...
		newDeps = append(newDeps, e)
	}
	if alreadyAdded {
		cnf.Dependencies = newDeps
		return s.UpdateConfig(cnf)
	}
	return nil
}
